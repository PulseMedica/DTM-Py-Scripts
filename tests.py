...
# Test of 'DTM.moveDirectoryHDF5FilesLocalToNAS()' function. Will return boolean True if tests passes, else False.
...
# Test of 'DTM.moveDirectoryHDF5FilesNAStoCloud()' function. Will return boolean True if tests passes, else False.
def MoveFilesToRemoteTest():

    # Create dummy files/folders.
    testStartDir = DTM.convertWindowsToUnixFilepath(DTM.createRandomSubfolder(os.getcwd(), random.randrange(12, 36)));
    testEndDir = DTM.convertWindowsToUnixFilepath(DTM.createRandomSubfolder(os.getcwd(), random.randrange(12, 36)));
    ...
    # Run test.
    try:
        DTM.moveDirectoryHDF5FilesNAStoCloud(testStartDir, testEndDir);
    except Exception as e:
        print("ERROR! DTM.moveDirectoryHDF5FilesNAStoCloud() threw exception: " + str(e));

    if(DTM.countFilesPerFolder(testStartDir) != 0 or DTM.countFilesPerFolder(testEndDir) != 2):
        print('ERROR! Could not successfully move test files.');
        return False;
...
if __name__ == '__main__':
    ...
    if(MoveFilesToRemoteTest() == False):
        print("MoveFilesToRemoteTest FAILED");
        exit(-3);
    else:
        print("MoveFilesToRemoteTest PASSED");
